...
NavigatorX is an experimental mobile navigation application built with React Native. It focuses on providing a smooth and responsive navigation experience by running its map-matching logic directly on the device, while delegating complex routing and pathfinding to a dedicated NavigatorX routing engine server. It combines a custom Go-powered native module with efficient data handling to provide real-time positioning and seamless navigation.
...
1.  **Run Build:**
    ...
    ```
    *Note: The `preview` profile is configured in `eas.json` to output a `.apk` file.*

---
*Built for high-precision navigation.*
